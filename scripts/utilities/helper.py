...
def create_type_of_delivery(input_df):
    ...
    assert (
        len(target_df.loc[target_df["order_type_id"] == -1]) == 0
    ), "Một số chuyến không xác định được loại vận chuyển"
...
QUERY_SQL_COMMAND_API = """
    ...
    carrier_information_final AS (
        SELECT 
            carrier_id, route_type, price, 
            status, is_priority_route, description, time_data, 
            time_display, rate, score, star, for_partner AS for_shop, 
            for_partner,
            price_ranking, speed_ranking, score_ranking
        FROM 
            carrier_information_final_tmp1
        WHERE 
        ...
        SELECT 
            ...
            price_ranking, speed_ranking, score_ranking
        FROM 
            carrier_information_priority_route
        WHERE 
            (SELECT row_count FROM row_count_check) >= 2   
        UNION ALL
        SELECT 
    ...
    SELECT * FROM carrier_information_final_tmp1 ORDER BY carrier_id; 
"""
...
QUERY_SQL_COMMAND_API_FINAL = """
        ...
        CASE
            ...
            WHEN carrier_id = 14 -- Lazada Logistics
                THEN
                    ...
                    END
        END AS redeem_fee, 
        status, ngn_status, is_priority_route, 
        n_days, 
        CASE
            WHEN ngn_status = 'Quá tải'
                THEN 
    ...
    carrier_information_tmp3 AS ( 
        ...
        status, ngn_status, is_priority_route, n_days, description, time_data, 
...
QUERY_SQL_COMMAND_STREAMLIT = """
    ...
    carrier_information_final AS (
        SELECT 
            carrier_id, new_type, price, insurance_fee, collection_fee, redeem_fee, total_price,
            status, is_priority_route, description, time_data, 
            time_display, rate, score, optimal_score, star, 
            for_shop, 
            for_partner,
