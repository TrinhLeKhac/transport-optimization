...
DF_STATUS_MAPPING = pd.DataFrame(
    data={
        "carrier_status": CHUA_GIAO_STATUS
        + DANG_GIAO_STATUS
        + HOAN_HANG_STATUS
        + THANH_CONG_STATUS
        + THAT_LAC_STATUS
        + KHONG_XET_STATUS,
        "status": ["Chưa giao hàng"] * len(CHUA_GIAO_STATUS)
        + ["Đang giao"] * len(DANG_GIAO_STATUS)
        + ["Hoàn hàng"] * len(HOAN_HANG_STATUS)
        + ["Thành công"] * len(THANH_CONG_STATUS)
        + ["Thất lạc"] * len(THAT_LAC_STATUS)
        + ["Không xét"] * len(KHONG_XET_STATUS),
    }
)
