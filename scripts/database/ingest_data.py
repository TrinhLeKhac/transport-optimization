...
config_init = [
    (
        ROOT_PATH + "/processed_data/phan_vung_nvc.parquet",
        "tbl_phan_vung_nvc",
        False,
        False,
        None,
    ),  # Đẩy lên database, nhưng không có trong câu query. Dùng để tổng hợp order_type, new_type trong tbl_order_type
    (
        ROOT_PATH + "/transform/buu_cuc.parquet",
        "tbl_buu_cuc",
        False,
        False,
        None,
    ),  # Đẩy lên database, nhưng không có trong câu query.
    (
        ROOT_PATH + "/output/data_order_type.parquet",
        "tbl_order_type",
        False,
        True,  # observe, trigger run.sh mode init khi file phan_vung_ghep_supership.xlsx thay đổi => phan_vung_nvc.parquet thay đổi => data_order_type.parquet thay đổi.
...
config_daily = [
    (
        ROOT_PATH + "/transform/chat_luong_noi_bo_njv.parquet",
        "tbl_clnb_ninja_van",
        False,
        False,
        None,
    ),  # observe, run.sh script mode daily khi file rst_cao_njv.xlsx thay đổi.
    (
        ROOT_PATH + "/transform/ngung_giao_nhan.parquet",
        "tbl_ngung_giao_nhan",
        False,
        False,  # observe, trigger rum.sh mode daily khi file ngung_giao_nhan.xlsx thay đổi.
        "carrier_id, sender_province_code, sender_district_code",
    ),
    (
        ROOT_PATH + "/transform/ngung_giao_nhan_level_3.parquet",
        "tbl_ngung_giao_nhan_level_3",
        False,
        False,  # observe, trigger rum.sh mode daily khi file shopee_ngung_giao_nhan.xlsx thay đổi.
        "carrier_id, receiver_province_code, receiver_district_code, receiver_commune_code",
    ),
    (
        ROOT_PATH + "/processed_data/danh_gia_zns_1_sao.parquet",
        "tbl_zns",
        False,
        False,
    ...
    (
        ROOT_PATH + "/output/service_fee.parquet",
        "tbl_service_fee",
        False,
        False,  # observe, trigger run.sh mode init khi file bang_cuoc_phi.xlsx thay đổi.
        "carrier_id, order_type, weight, pickup",
    ),
    (
        ROOT_PATH + "/output/priority_route_api.parquet",
        "tbl_priority_route",
        True,
        False,
        "carrier_id, sender_province_code, sender_district_code, receiver_province_code, receiver_district_code",
    ...
    (
        ROOT_PATH + "/output/data_api.parquet",
        "tbl_data_api",
        True,
        False,
...
def create_index_of_table(schema_name, tbl_name, idx_cols=None):
    # Create connection
    connection = psycopg2.connect(settings.SQLALCHEMY_DATABASE_URI)
    ...
    create_idx_query = (
        f"CREATE UNIQUE INDEX {tbl_name}_idx ON {schema_name}.{tbl_name}({idx_cols})"
    )
...
def ingest_tbl_to_db(
    ...
    if mode == "append":
        try:
            ...
            print(
                "Table '{}' does not exist in schema '{}'.".format(
