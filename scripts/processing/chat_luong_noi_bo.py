...
def xu_ly_chat_luong_noi_bo_ninja_van_old():
    # 1. Đọc data raw
    try:
        clnb_njv_df = pd.read_excel(
            ROOT_PATH + "/user_input/chat_luong_noi_bo_njv.xlsx"
        )
    ...
    clnb_njv_df["id_receiver_district"] = clnb_njv_df["id_receiver_district"].astype(
        str
    )
    clnb_njv_df["id_receiver_district"] = clnb_njv_df["id_receiver_district"].str.zfill(
        3
    )

    clnb_njv_df = clnb_njv_df.merge(
    ...
    # 3. Tách percentage
    clnb_njv_df["delivery_success_rate"] = (
        clnb_njv_df["delivery_success_rate"].astype(str).apply(get_pct_ninja_van)
    )

    # 4. Chuẩn hóa tên quận/huyện, tỉnh/thành
    clnb_njv_df = normalize_province_district(
        clnb_njv_df, tinh_thanh="receiver_province", quan_huyen="receiver_district"
    )
    clnb_njv_df = clnb_njv_df.loc[
        clnb_njv_df["receiver_province"].notna()
        & clnb_njv_df["receiver_district"].notna()
    ]
...
def xu_ly_chat_luong_noi_bo_ninja_van():
    ...
    raw_njv["njv_post_office"] = (
        raw_njv["njv_post_office"].astype(str).apply(lambda x: x.replace("\xa0", " "))
    )

    new_njv = raw_njv.sort_values(
        "delivery_failed_rate", ascending=False
    ).drop_duplicates(subset=["receiver_province", "njv_post_office"], keep="first")

    # 3. Xử lý phần giao nhau
    new_njv1 = old_njv.merge(
        new_njv[["njv_post_office", "delivery_failed_rate"]],
        on="njv_post_office",
        how="inner",
    )
    ...
    # 4. Xử lý phần riêng
    new_njv2 = new_njv.loc[
        ~new_njv["njv_post_office"].isin(new_njv1["njv_post_office"])
    ]
    ...
    new_njv2 = normalize_province_district(
        new_njv2, tinh_thanh="receiver_province", quan_huyen="receiver_district"
    )
    new_njv2 = new_njv2.loc[
        new_njv2["receiver_province"].notna() & new_njv2["receiver_district"].notna()
    ]
    ...
    clnb_njv_df = clnb_njv_df.drop_duplicates(
        subset=["receiver_province", "receiver_district"], keep="first"
    )
    clnb_njv_df.to_parquet(
        ROOT_PATH + "/processed_data/chat_luong_noi_bo_njv.parquet", index=False
    )
...
def xu_ly_chat_luong_noi_bo_lazada():
    try:
        clnb_lex_df = pd.read_excel(
            ROOT_PATH + "/user_input/chat_luong_noi_bo_lex.xlsx", sheet_name="Province"
        )
    except FileNotFoundError:
        print(
        ...
        clnb_lex_df = pd.read_excel(
            ROOT_PATH + "/input/chat_luong_noi_bo_lex.xlsx", sheet_name="Province"
        )
    ...
    clnb_lex_df["mean_w"] = clnb_lex_df[
        ["w1", "w2", "w3", "w4", "w5", "w6", "w7", "w8"]
    ].mean(axis=1)
    clnb_lex_df = (
        clnb_lex_df.groupby(["receiver_province", "receiver_district"])
        .agg(delivery_success_rate=("mean_w", "mean"))
        .reset_index()
    )

    clnb_lex_df = normalize_province_district(
        clnb_lex_df, tinh_thanh="receiver_province", quan_huyen="receiver_district"
    )
    clnb_lex_df = clnb_lex_df.loc[
        clnb_lex_df["receiver_province"].notna()
        & clnb_lex_df["receiver_district"].notna()
    ]
    clnb_lex_df = clnb_lex_df.drop_duplicates(
        ["receiver_province", "receiver_district"], keep="first"
    )
    ...
    clnb_lex_df = (
        ...
        .rename(
            columns={"province": "receiver_province", "district": "receiver_district"}
        )
        .merge(clnb_lex_df, on=["receiver_province", "receiver_district"], how="left")
    )
    clnb_lex_df["delivery_success_rate"] = clnb_lex_df["delivery_success_rate"].fillna(
        mean_value
    )
    clnb_lex_df.to_parquet(
        ROOT_PATH + "/processed_data/chat_luong_noi_bo_lazada.parquet", index=False
    )
