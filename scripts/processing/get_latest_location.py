...
def get_latest_province_mapping_district():
    province_api_res = requests.get(
        "https://api.mysupership.vn/v1/partner/areas/province", verify=False
    ).json()
    ...
    for province_code in province_df["province_code"].tolist():
        district_api_res = requests.get(
            "https://api.mysupership.vn/v1/partner/areas/district?province={}".format(
                province_code
            ),
            verify=False,
        ).json()["results"]
    ...
    province_district_df = (
        province_df.merge(district_df, on="province_code", how="inner")
        .sort_values(["province_code", "district_code"])
        .reset_index(drop=True)
    )
    province_district_df.to_parquet(
        ROOT_PATH + "/user_input/province_mapping_district.parquet", index=False
    )
...
def get_latest_province_mapping_district_json():
    ...
    with open(
        ROOT_PATH + "/user_input/province_mapping_district_from_api.json", "w"
    ) as file:
...
def get_latest_province_mapping_district_mapping_ward():
    ...
    list_commune_df = []
    for district, district_code in zip(
        district_df["district"].tolist(), district_df["district_code"].tolist()
    ):
        commune_api_res = requests.get(
            "https://api.mysupership.vn/v1/partner/areas/commune?district={}".format(
                district_code
            ),
            verify=False,
        ).json()["results"]
    ...
    province_district_ward_df = (
        province_df.merge(district_df, on="province_code", how="inner")
        .merge(commune_df, on="district_code", how="inner")
        .sort_values(
            ["province", "district", "commune", "commune_code"],
    ...
    province_district_ward_df.to_parquet(
...
def get_latest_province_mapping_district_mapping_ward_json():
    ...
    for province_code, province in zip(
        ...
        province_mapping_district_mapping_ward_dict[province] = (
            district_mapping_commune_dict
        )
