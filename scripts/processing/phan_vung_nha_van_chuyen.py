...
def xu_ly_phan_vung_nha_van_chuyen():
    # 1. Đọc thông tin raw
    try:
        phan_vung_nvc = pd.read_excel(
            ROOT_PATH + "/user_input/phan_vung_ghep_supership.xlsx", dtype=str
        )
    except FileNotFoundError:
        print(
            f"Error: The file {ROOT_PATH}/user_input/phan_vung_ghep_supership.xlsx was not found. Use file {ROOT_PATH}/input/phan_vung_ghep_supership.xlsx instead."
        )
        phan_vung_nvc = pd.read_excel(
            ROOT_PATH + "/input/phan_vung_ghep_supership.xlsx", dtype=str
        )
    ...
    phan_vung_nvc.columns = [
        ...
        "ghtk_outer_region",
        "ghtk_inner_region",
        "supership_outer_region",
        "supership_inner_region",
        "tikinow_outer_region",
        "tikinow_inner_region",
        "vnpost_outer_region",
        "vnpost_inner_region",
        "lazada_outer_region",
        "lazada_inner_region",
    ...
    ghn = phan_vung_nvc[
        ...
        columns={"ghn_outer_region": "outer_region", "ghn_inner_region": "inner_region"}
    )
    ghn["carrier"] = "GHN"
    ...
    njv = phan_vung_nvc[
        ...
        columns={"njv_outer_region": "outer_region", "njv_inner_region": "inner_region"}
    )
    ...
    vtp = phan_vung_nvc[
        ...
        columns={"vtp_outer_region": "outer_region", "vtp_inner_region": "inner_region"}
    )
    ...
    spx = phan_vung_nvc[
        ...
        columns={"spx_outer_region": "outer_region", "spx_inner_region": "inner_region"}
    )
    ...
    phan_vung_nvc_final = pd.concat(
        [ghn, njv, vtp, spx, best, ghtk, vnpost, lazada], ignore_index=True
    )
    ...
    phan_vung_nvc_final["inner_region"] = (
        phan_vung_nvc_final["inner_region"]
        .map({"Nội thành": "Nội Thành", "Ngoại thành": "Ngoại Thành"})
        .fillna("inner_region")
    )
    ...
    # 5. Mapping ID column
    phan_vung_nvc_final = phan_vung_nvc_final.merge(
    ...
    phan_vung_nvc_final["inner_region_id"] = phan_vung_nvc_final["inner_region"].map(
        {"Nội Thành": 0, "Ngoại Thành": 1}
    )
    phan_vung_nvc_final["carrier_id"] = phan_vung_nvc_final["carrier"].map(
        MAPPING_CARRIER_ID
    )
    phan_vung_nvc_final = phan_vung_nvc_final[
        [
    ...
    # 6. Lưu thông tin
    phan_vung_nvc_final.to_parquet(
        ROOT_PATH + "/processed_data/phan_vung_nvc.parquet", index=False
    )
