...
COLUMNS_CUOC_PHI = (
    ["gt", "lt_or_eq"] + list(MAPPING_ORDER_TYPE_ID.keys()) + ["Liên Thành"]
)
...
def xu_ly_bang_gia_cuoc():
    ...
    ninja_van_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 2:13]], axis=1
    ...
    best_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 13:24]], axis=1
    ).reset_index(drop=True)
    ...
    shopee_express_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 24:35]], axis=1
    ).reset_index(drop=True)
    ...
    ghn_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 35:46]], axis=1
    ).reset_index(drop=True)
    ...
    viettel_post_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 46:57]], axis=1
    ).reset_index(drop=True)
    ...
    ghtk_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 57:68]], axis=1
    ).reset_index(drop=True)
    ...
    vnpost_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 79:90]], axis=1
    ).reset_index(drop=True)
    ...
    lazada_df = pd.concat(
        [bang_gia_cuoc_df.iloc[1:101, :2], bang_gia_cuoc_df.iloc[1:101, 90:101]], axis=1
    ).reset_index(drop=True)
