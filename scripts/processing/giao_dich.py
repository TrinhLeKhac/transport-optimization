...
def xu_ly_giao_dich():
    ...
    best_df = pd.read_excel(
        ROOT_PATH + "/input/giao_dich_nha_van_chuyen.xlsx", sheet_name="BEST"
    )
    ...
    best_df["carrier_delivered_at"] = pd.to_datetime(
        best_df["carrier_delivered_at"], errors="coerce"
    )

    best_df["carrier_delivered_at"] = (
        best_df["carrier_delivered_at"]
        .apply(lambda x: str(x))
        .apply(convert_datetime_type1)
    )
    best_df["created_at"] = (
        best_df["created_at"].apply(lambda x: str(x)).apply(convert_datetime_type2)
    )

    print("Xử lý giao dịch Ninja Van...")
    njv_df = pd.read_excel(
        ROOT_PATH + "/input/giao_dich_nha_van_chuyen.xlsx", sheet_name="NJV"
    )
    ...
    njv_df["carrier_delivered_at"] = pd.to_datetime(
        njv_df["carrier_delivered_at"], errors="coerce"
    )

    njv_df["carrier_delivered_at"] = (
        njv_df["carrier_delivered_at"]
        .apply(lambda x: str(x))
        .apply(convert_datetime_type1)
    )
    ...
    ghn_df = pd.read_excel(
        ROOT_PATH + "/input/giao_dich_nha_van_chuyen.xlsx", sheet_name="GHN"
    )
    ...
    ghn_df["carrier_delivered_at"] = pd.to_datetime(
        ghn_df["carrier_delivered_at"], errors="coerce"
    )

    ghn_df["carrier_delivered_at"] = (
        ghn_df["carrier_delivered_at"]
        .apply(lambda x: str(x))
        .apply(convert_datetime_type1)
    )
    ...
    vtp_df = pd.read_excel(
        ROOT_PATH + "/input/giao_dich_nha_van_chuyen.xlsx", sheet_name="VTP"
    )
    ...
    vtp_df["carrier_delivered_at"] = pd.to_datetime(
        vtp_df["carrier_delivered_at"], errors="coerce"
    )

    vtp_df["carrier_delivered_at"] = (
        vtp_df["carrier_delivered_at"]
        .apply(lambda x: str(x))
        .apply(convert_datetime_type1)
    )
    ...
    spx_df = pd.read_excel(
        ROOT_PATH + "/input/giao_dich_nha_van_chuyen.xlsx", sheet_name="SPX"
    )
    ...
    spx_df["carrier_delivered_at"] = pd.to_datetime(
        spx_df["carrier_delivered_at"], errors="coerce"
    )

    spx_df["carrier_delivered_at"] = (
        spx_df["carrier_delivered_at"]
        .apply(lambda x: str(x))
        .apply(convert_datetime_type1)
    )
    ...
    ghtk_df = pd.read_excel(
        ROOT_PATH + "/input/giao_dich_nha_van_chuyen.xlsx", sheet_name="GHTK"
    )
    ...
    ghtk_df["carrier_delivered_at"] = pd.to_datetime(
        ghtk_df["carrier_delivered_at"], errors="coerce"
    )

    ghtk_df["carrier_delivered_at"] = (
        ghtk_df["carrier_delivered_at"]
        .apply(lambda x: str(x))
        .apply(convert_datetime_type1)
    )
    ...
    raw_order_df = pd.concat(
        [best_df, njv_df, ghn_df, vtp_df, spx_df, ghtk_df], ignore_index=True
    )
    raw_order_df["carrier_status"] = raw_order_df["carrier_status"].str.strip()

    print("Lưu thông tin...")
    raw_order_df.to_parquet(
        ROOT_PATH + "/processed_data/raw_order.parquet", index=False
    )


@exception_wrapper
def xu_ly_giao_dich_co_khoi_luong():
    order_with_weight_df = pd.read_excel(
        ROOT_PATH + "/input/don_co_khoi_luong.xlsx", sheet_name="Combined"
    )

    order_with_weight_df = order_with_weight_df[
        ["Mã Đơn SuperShip", "Khối Lượng", "Kho Hàng"]
    ]
    order_with_weight_df.columns = ["order_code", "weight", "storage_address"]

    print("Lưu thông tin...")
    order_with_weight_df.to_parquet(
        ROOT_PATH + "/processed_data/order_with_weight.parquet", index=False
    )


@exception_wrapper
def tong_hop_thong_tin_giao_dich(
    run_date_str, from_api=True, n_days_back=30, save_output=True
):
    ...
    if not from_api:
        ...
        order_with_weight_df = pd.read_parquet(
            ROOT_PATH + "/processed_data/order_with_weight.parquet"
        )

        print("Combine thông tin giao dịch")
        order_df = raw_order_df.merge(
            order_with_weight_df, on="order_code", how="inner"
        )
        print("Số giao dịch hợp lệ: ", len(order_df))

        print("Tách địa chỉ tỉnh/thành, quận/huyện lấy hàng từ kho nhận")
        order_df["sender_province"] = (
            order_df["storage_address"].str.split(", ").str[-1]
        )
        order_df["sender_district"] = (
            order_df["storage_address"].str.split(", ").str[-2]
        )

        print("Chuẩn hóa thông tin tỉnh/thành, quận/huyện lấy hàng...")
        order_df = normalize_province_district(
            order_df, tinh_thanh="sender_province", quan_huyen="sender_district"
        )

        print("Chuẩn hóa thông tin tỉnh/thành, quận/huyện giao hàng...")
        order_df = normalize_province_district(
            order_df, tinh_thanh="receiver_province", quan_huyen="receiver_district"
        )
        ...
        print(
            "Số giao dịch sau khi chuẩn hóa tỉnh/thành, quận/huyện: ",
            len(valid_order_df),
        )

        valid_order_df = valid_order_df.merge(
        ...
        valid_order_df["carrier_id"] = valid_order_df["carrier"].map(MAPPING_CARRIER_ID)
        ...
        valid_order_df["created_at"] = pd.to_datetime(
            valid_order_df["created_at"], errors="coerce"
        )
        valid_order_df["sent_at"] = pd.to_datetime(
            valid_order_df["sent_at"], errors="coerce"
        )
        valid_order_df["picked_at"] = pd.to_datetime(
            valid_order_df["picked_at"], errors="coerce"
        )
        valid_order_df["carrier_updated_at"] = pd.to_datetime(
            valid_order_df["carrier_updated_at"], errors="coerce"
        )
        valid_order_df["last_delivering_at"] = pd.to_datetime(
            valid_order_df["last_delivering_at"], errors="coerce"
        )
        valid_order_df["carrier_delivered_at"] = pd.to_datetime(
            valid_order_df["carrier_delivered_at"], errors="coerce"
        )
        ...
        assert (
            set_carrier - set_norm_full_carrier == set()
        ), "Ops, Tên nhà vận chuyển chưa được chuẩn hóa"
        ...
        valid_order_df = valid_order_df.loc[
            valid_order_df["created_at"] >= (run_date - timedelta(days=n_days_back))
        ]
        valid_order_df = valid_order_df.sort_values(
        ...
        valid_order_df["delivery_type"] = valid_order_df["pickup"].map(
            {"0": "Gửi Bưu Cục", "1": "Lấy Tận Nơi"}
        )
        valid_order_df["is_returned"] = valid_order_df["barter"].map(
            {"0": False, "1": True}
        )

        valid_order_df = valid_order_df.rename(
            columns={
        ...
        valid_order_df = valid_order_df.merge(
            ...
            on=["sender_province_code", "sender_district_code"],
            how="left",
        ).merge(
            PROVINCE_MAPPING_DISTRICT_DF.rename(
                columns={
            ...
            on=["receiver_province_code", "receiver_district_code"],
            how="left",
        )
        ...
        valid_order_df = valid_order_df.sort_values(
            ["order_code", "created_at"], ascending=[True, False]
        ).drop_duplicates("order_code", keep="first")
    ...
    if save_output:
        ...
        valid_order_df.to_parquet(
            ROOT_PATH + "/processed_data/order.parquet", index=False
        )
    else:
