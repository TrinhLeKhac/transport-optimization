...
def xu_ly_danh_gia_zns(run_date_str, from_api=True, n_days_back=30):
    ...
    if not from_api:
        ...
        danh_gia_zns_df["reviewed_at"] = pd.to_datetime(
            danh_gia_zns_df["reviewed_at"], errors="coerce"
        )
        danh_gia_zns_df["date"] = pd.to_datetime(
            danh_gia_zns_df["date"], errors="coerce"
        )

        # 2. Chuẩn hóa tên quận/huyện, tỉnh/thành
        danh_gia_zns_df = normalize_province_district(
            danh_gia_zns_df,
        ...
        danh_gia_zns_df.loc[
            danh_gia_zns_df["carrier"] == "Shopee Express", "carrier"
        ] = "SPX Express"
        ...
        # 4. Lưu thông tin
        danh_gia_zns_df.to_parquet(
            ROOT_PATH + "/processed_data/danh_gia_zns.parquet", index=False
        )
        ...
        danh_gia_zns_df["carrier"] = danh_gia_zns_df["carrier_id"].map(
            MAPPING_ID_CARRIER
        )
        danh_gia_zns_df = danh_gia_zns_df.rename(
            columns={
                "receiver_province": "receiver_province_code",
                "receiver_district": "receiver_district_code",
            }
        )
        danh_gia_zns_df = danh_gia_zns_df.merge(
        ...
        danh_gia_zns_df = danh_gia_zns_df.loc[
            danh_gia_zns_df["reviewed_at"] >= (run_date - timedelta(days=n_days_back))
        ]
...
def xu_ly_danh_gia_zns_1_sao(run_date_str):
    ...
    danh_gia_zns_1sao = danh_gia_zns_1sao[
        ["receiver_province", "receiver_district", "carrier", "n_days"]
    ]
    danh_gia_zns_1sao = danh_gia_zns_1sao.sort_values(
        ["receiver_province", "receiver_district", "carrier", "n_days"],
        ascending=[True, True, True, True],
    )
    danh_gia_zns_1sao = danh_gia_zns_1sao.drop_duplicates(
        ["receiver_province", "receiver_district", "carrier"], keep="first"
    )
    danh_gia_zns_1sao = PROVINCE_MAPPING_DISTRICT_CROSS_CARRIER_DF.merge(
        danh_gia_zns_1sao,
    ...
    danh_gia_zns_1sao["carrier_id"] = danh_gia_zns_1sao["carrier"].map(
        MAPPING_CARRIER_ID
    )
    danh_gia_zns_1sao = danh_gia_zns_1sao[
        ["carrier_id", "receiver_province_code", "receiver_district_code", "n_days"]
    ]

    danh_gia_zns_1sao.to_parquet(
        ROOT_PATH + "/processed_data/danh_gia_zns_1_sao.parquet", index=False
    )
