...
class Authenticator:
    ...
    def __init__(self):
        ...
        self.usernames = [
            v["username"] for v in self.config["credentials"]["names"].values()
        ]
        self.passwords = [
            v["password"] for v in self.config["credentials"]["names"].values()
        ]
    ...
    def _set_exp_date(self) -> int:
        ...
        """
        expire = int(
            (datetime.now() + timedelta(seconds=self.cookie_expiry_day)).strftime(
                "%Y%m%d%H%M%S"
            )
        )
    ...
    def _jwt_decode(self) -> str:
        ...
        try:
            return jwt.decode(
                self.encoded_jwt, self.cookie_signature_key, algorithms=["HS256"]
            )
        except:
            return False
        ...
        if st.session_state["username"] in self.usernames:
            try:
                if (
                    ...
                    == self.authentication[st.session_state["username"]]
                ):
                    if inplace:
