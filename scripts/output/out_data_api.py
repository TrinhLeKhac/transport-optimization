...
def customer_best_carrier(data_api_df):
    data_api_df["combine_col"] = data_api_df[
        ["delivery_success_rate", "total_order"]
    ].apply(tuple, axis=1)

    data_api_df["delivery_success_rate_id"] = (
        data_api_df.groupby(
            ["receiver_province_code", "receiver_district_code", "order_type_id"]
        )["combine_col"]
    ...
    data_api_df["wscore"] = (
        data_api_df["fastest_carrier_id"] * 1.4
...
def calculate_vpn(group):
    ...
    return pd.Series(
        {
            "time_data_modified": time_data,
            "score_modified": score,
            "rate_modified": rate,
        }
    )
...
def modified_output_api(df_api_full):
    ...
    vnp_data_api = (
        main_data_api.groupby(
            ["receiver_province_code", "receiver_district_code", "new_type"]
        )
        .apply(calculate_vpn)
        .reset_index()
    )
    ...
    df_api_full_final = df_api_full.merge(
        vnp_data_api,
        on=[
            "carrier_id",
            "receiver_province_code",
    ...
    df_api_full_final.loc[df_api_full_final["carrier_id"] == 13, "time_data"] = (
        df_api_full_final["time_data_modified"]
    )
    df_api_full_final.loc[df_api_full_final["carrier_id"] == 13, "score"] = (
        df_api_full_final["score_modified"]
    )
    df_api_full_final.loc[df_api_full_final["carrier_id"] == 13, "rate"] = (
        df_api_full_final["rate_modified"]
    )
    df_api_full_final = df_api_full_final.drop(
        ["time_data_modified", "score_modified", "rate_modified"], axis=1
    )

    # Replace old value Lazada Logistics by new value
    df_api_full_final = df_api_full_final.merge(
        lex_data_api,
        on=[
            "carrier_id",
            "receiver_province_code",
    ...
    df_api_full_final.loc[df_api_full_final["carrier_id"] == 14, "time_data"] = (
        df_api_full_final["time_data_modified"]
    )
    df_api_full_final.loc[df_api_full_final["carrier_id"] == 14, "score"] = (
        df_api_full_final["score_modified"]
    )
    df_api_full_final.loc[df_api_full_final["carrier_id"] == 14, "rate"] = (
        df_api_full_final["rate_modified"]
    )
    df_api_full_final = df_api_full_final.drop(
        ["time_data_modified", "score_modified", "rate_modified"], axis=1
...
def out_data_api(
    ...
    qua_tai1 = ngung_giao_nhan.loc[
        ngung_giao_nhan["score"].isin(OVERLOADING_SCORE_DICT["Ngưng giao nhận"])
    ]
    qua_tai1 = qua_tai1[
        ["receiver_province", "receiver_district", "carrier", "status"]
    ].rename(columns={"status": "carrier_status_comment"})
    ...
    qua_tai2.loc[
        qua_tai2["carrier_status_comment"] == "Loại", "carrier_status_comment"
    ] = "Tổng số đánh giá ZNS 1, 2 sao >= 30% tổng đơn"
    ...
    qua_tai4 = chat_luong_noi_bo.loc[
        chat_luong_noi_bo["score"].isin(OVERLOADING_SCORE_DICT["Chất lượng nội bộ"])
    ]
    qua_tai4["carrier_status_comment"] = qua_tai4["status"] + " (clnb)"
    qua_tai4 = qua_tai4[
        ["receiver_province", "receiver_district", "carrier", "carrier_status_comment"]
    ]

    qua_tai5 = thoi_gian_giao_hang.loc[
        thoi_gian_giao_hang["score"].isin(OVERLOADING_SCORE_DICT["Thời gian giao hàng"])
    ]
    qua_tai5["carrier_status_comment"] = (
        qua_tai5["status"] + " (" + qua_tai5["order_type"] + ")"
    )
    qua_tai5 = qua_tai5[
        ["receiver_province", "receiver_district", "carrier", "carrier_status_comment"]
    ]

    qua_tai6 = kho_giao_nhan.loc[
        kho_giao_nhan["score"].isin(OVERLOADING_SCORE_DICT["Có kho giao nhận"])
    ]
    qua_tai6 = qua_tai6[
        ["receiver_province", "receiver_district", "carrier", "status"]
    ].rename(columns={"status": "carrier_status_comment"})

    qua_tai7 = don_ton_dong.loc[
        don_ton_dong["score"].isin(OVERLOADING_SCORE_DICT["Đơn tồn đọng"])
    ]
    qua_tai7 = qua_tai7[
        ["receiver_province", "receiver_district", "carrier", "carrier_status_comment"]
    ]
    ...
    qua_tai = pd.concat(
        [qua_tai1, qua_tai2, qua_tai3, qua_tai4, qua_tai5, qua_tai6, qua_tai7]
    )

    qua_tai = (
        qua_tai.groupby(["receiver_province", "receiver_district", "carrier"])[
            "carrier_status_comment"
        ]
    ...
    thoi_gian_giao_hang = thoi_gian_giao_hang.rename(
        columns={"delivery_time_mean_h": "estimate_delivery_time_details"}
    )
    thoi_gian_giao_hang["estimate_delivery_time_details"] = np.round(
        thoi_gian_giao_hang["estimate_delivery_time_details"] / 24, 2
    )
    thoi_gian_giao_hang["estimate_delivery_time"] = thoi_gian_giao_hang[
        "estimate_delivery_time_details"
    ].apply(round_value)

    # Thời gian giao hàng default
    active_carrier_df = pd.DataFrame(
        data={
            "carrier": carriers,
        }
    )
    ...
    thoi_gian_giao_hang_default = (
        ...
        .merge(active_carrier_df.merge(loai_van_chuyen_df, how="cross"), how="cross")
    )

    thoi_gian_giao_hang_final = thoi_gian_giao_hang_default.merge(
        thoi_gian_giao_hang,
    ...
    thoi_gian_giao_hang_final.loc[
        thoi_gian_giao_hang_final["total_order"] == 0, "estimate_delivery_time"
    ] = thoi_gian_giao_hang_final["default_delivery_time"]

    thoi_gian_giao_hang_final.loc[
        thoi_gian_giao_hang_final["total_order"] == 0, "estimate_delivery_time_details"
    ...
    for focus_df in [
        ...
        score_df_list.append(
            target_df[
                ["receiver_province", "receiver_district", "carrier", "weight_score"]
            ]
        )

    score_df = pd.concat(score_df_list, ignore_index=False)
    score_final = (
        score_df.groupby(["receiver_province", "receiver_district", "carrier"])[
            "weight_score"
        ]
        .mean()
        .reset_index()
    ...
    api_data_final["delivery_success_rate"] = np.round(
        api_data_final["delivery_success_rate"] * 100, 2
    )
    ...
    success_rate_modified_1["delivery_success_rate_modified"] = np.round(
        np.random.uniform(80, 85, len(success_rate_modified_1)), 2
    )
    api_data_final = api_data_final.merge(
        success_rate_modified_1,
        on=["receiver_province", "receiver_district", "carrier"],
        how="left",
    )
    api_data_final.loc[api_data_final["total_order"] == 0, "delivery_success_rate"] = (
        api_data_final["delivery_success_rate_modified"]
    )
    ...
    success_rate_modified_2 = api_data_final.loc[
        api_data_final["delivery_success_rate"] == 100
    ...
    api_data_final = api_data_final.merge(
        success_rate_modified_2,
        on=["receiver_province", "receiver_district", "carrier"],
        how="left",
    )
    api_data_final.loc[
        api_data_final["delivery_success_rate"] == 100, "delivery_success_rate"
    ] = api_data_final["delivery_success_rate_modified"]
    api_data_final.drop("delivery_success_rate_modified", axis=1, inplace=True)

    api_data_final["combine_col"] = api_data_final[
        ["delivery_success_rate", "total_order"]
    ].apply(tuple, axis=1)
    api_data_final["delivery_success_rate_id"] = (
        api_data_final.groupby(
            ["receiver_province", "receiver_district", "order_type"]
        )["combine_col"]
        .rank(method="dense", ascending=False)
        .astype(int)
    )
    ...
    api_data_final = api_data_final.merge(
        qua_tai, on=["receiver_province", "receiver_district", "carrier"], how="left"
    )
    api_data_final.loc[api_data_final["total_order"] == 0, "carrier_status_comment"] = (
        "Không có đơn hàng"
    )
    api_data_final["carrier_status_comment"] = api_data_final[
        "carrier_status_comment"
    ].fillna("Bình thường")

    api_data_final["carrier_status"] = 2
    api_data_final.loc[
        api_data_final["carrier_status_comment"].isin(
            ["Bình thường", "Không có đơn hàng"]
        ),
        "carrier_status",
    ] = 0
    api_data_final.loc[
        api_data_final["carrier_status_comment"].str.contains("Quá tải"),
        "carrier_status",
    ] = 1
    ...
    api_data_final["order_type_id"] = api_data_final["order_type"].map(
        MAPPING_ORDER_TYPE_ID
    )

    if show_logs:
        print("7. Thông tin nhà vận chuyển nhanh nhất, hiệu quả nhất")
    api_data_final["fastest_carrier_id"] = api_data_final.groupby(
        ["receiver_province_code", "receiver_district_code", "order_type_id"]
    )["estimate_delivery_time_details"].rank(method="dense", ascending=True)
    api_data_final["fastest_carrier_id"] = api_data_final["fastest_carrier_id"].astype(
        int
    )

    api_data_final["highest_score_carrier_id"] = api_data_final.groupby(
        ["receiver_province_code", "receiver_district_code", "order_type_id"]
    )["score"].rank(method="dense", ascending=False)
    api_data_final["highest_score_carrier_id"] = api_data_final[
        "highest_score_carrier_id"
    ].astype(int)
    ...
    zns_df = (
        zns_df.groupby(["receiver_province", "receiver_district", "carrier"])
        .agg(star=("n_stars", "mean"))
        .reset_index()
    )
    zns_df["star"] = np.round(zns_df["star"], 1)

    api_data_final = api_data_final.merge(
        zns_df, on=["receiver_province", "receiver_district", "carrier"], how="left"
    )
    ...
    api_data_final["sys_order_type_id"] = api_data_final["order_type_id"].map(
    ...
    api_data_final["sys_order_type_id"] = api_data_final["sys_order_type_id"].astype(
        str
    )
...
def assign_supership_carrier(df_api, save_output=True):
    ...
    df_supership = (
        time_data.merge(
            rate,
            on=["receiver_province_code", "receiver_district_code", "new_type"],
            how="inner",
        )
        .merge(
            score,
    ...
    df_api_full["speed_ranking"] = df_api_full.groupby(
        ["receiver_province_code", "receiver_district_code", "new_type"]
    )["time_data"].rank(method="dense", ascending=True)
    df_api_full["speed_ranking"] = df_api_full["speed_ranking"].astype(int)

    df_api_full["score_ranking"] = df_api_full.groupby(
        ["receiver_province_code", "receiver_district_code", "new_type"]
    )["score"].rank(method="dense", ascending=False)
    df_api_full["score_ranking"] = df_api_full["score_ranking"].astype(int)

    df_api_full["combine_col"] = df_api_full[["rate", "total_order"]].apply(
        tuple, axis=1
    )
    ...
    df_api_full["wscore"] = (
        df_api_full["speed_ranking"] * 1.4
        + df_api_full["rate_ranking"] * 1.2
