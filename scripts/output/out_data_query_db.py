...
# @exception_wrapper
def out_data_order_type(
    carriers=ACTIVE_CARRIER, include_supership=True, show_logs=True
):
    ...
    carrier_id_df = pd.DataFrame(
        data={"carrier_id": [MAPPING_CARRIER_ID[v] for v in carriers]}
    )
    ...
    input_df = carrier_id_df.merge(sender_df, how="cross").merge(
        receiver_df, how="cross"
    )
    ...
    order_type_df = order_type_df.rename(
...
def out_data_service_fee():
    ...
    cuoc_phi_df = cuoc_phi_df[
        ["carrier_id", "lt_or_eq", "order_type", "order_type_id", "service_fee"]
    ].rename(
        columns={
            "lt_or_eq": "weight",
    ...
    service_fee_df.loc[
        (service_fee_df["pickup"] == "1") & (service_fee_df["carrier_id"] == 7), "price"
    ] = (service_fee_df["price"] + 1500)
    service_fee_df.loc[
        (service_fee_df["pickup"] == "1") & (service_fee_df["carrier_id"] == 2), "price"
    ] = (service_fee_df["price"] + 1000)

    service_fee_df = service_fee_df[
        ["carrier_id", "order_type", "new_type", "pickup", "weight", "price"]
    ]
