...
def get_metadata_tuyen_uu_tien(run_date_str, n_months_back=2):
    ...
    df_target = df_target.loc[
        ...
        & (
            df_target["last_delivering_at"]
            >= (run_date - relativedelta(months=n_months_back))
        )
        & (df_target["last_delivering_at"] < run_date)
    ]
    df_target["delta_hour"] = (
        (df_target["last_delivering_at"] - df_target["picked_at"]).dt.total_seconds()
        / 60
        / 60
    )

    get_top_by_15_pct_df = df_target.groupby("order_type_id", group_keys=False).apply(
        get_top_percent, sort_col="delta_hour", ascending=True, pct=0.15
    )
    get_top_by_15_pct_df["type"] = "top 15%"

    get_top_by_20_pct_df = df_target.groupby("order_type_id", group_keys=False).apply(
        get_top_percent, sort_col="delta_hour", ascending=True, pct=0.2
    )
    get_top_by_20_pct_df["type"] = "top 20%"

    get_top_by_25_pct_df = df_target.groupby("order_type_id", group_keys=False).apply(
        get_top_percent, sort_col="delta_hour", ascending=True, pct=0.25
    )
    get_top_by_25_pct_df["type"] = "top 25%"

    get_top_by_pct_df = pd.concat(
        [get_top_by_15_pct_df, get_top_by_20_pct_df, get_top_by_25_pct_df]
    )
    meta_priority_route_df = (
        get_top_by_pct_df.groupby(["order_type_id", "type"])["delta_hour"]
        .mean()
        .reset_index()
    )
    meta_priority_route_df["rounded_delta_hour"] = np.round(
        meta_priority_route_df["delta_hour"], 2
    )
    meta_priority_route_df = meta_priority_route_df.sort_values(
        ["type", "delta_hour"]
    ).reset_index(drop=True)

    meta_priority_route_df["idea_delta_hour"] = meta_priority_route_df[
        "rounded_delta_hour"
    ]
    meta_priority_route_df.loc[
        meta_priority_route_df["rounded_delta_hour"] < 24, "idea_delta_hour"
    ] = 24
    meta_priority_route_df["order_type"] = meta_priority_route_df["order_type_id"].map(
    ...
    meta_priority_route_df.to_parquet(
        ROOT_PATH + "/processed_data/meta_priority_route.parquet", index=False
    )
...
def get_data_tuyen_uu_tien(run_date_str, n_months_back=2, priority_type="top 25%"):
    ...
    df_target.loc[
        (df_target["last_delivering_at"] < run_date)
        & (df_target["last_delivering_at"] >= (run_date - relativedelta(months=1))),
        "is_1_month",
    ] = True
    df_target.loc[
        (df_target["last_delivering_at"] < run_date)
    ...
    df_target.loc[
        (df_target["last_delivering_at"] < run_date)
        & (df_target["last_delivering_at"] >= (run_date - relativedelta(months=2))),
        "is_2_month",
    ] = True
    df_target.loc[
        (df_target["last_delivering_at"] < run_date)
    ...
    meta_priority_route_df = pd.read_parquet(
        ROOT_PATH + "/processed_data/meta_priority_route.parquet"
    )
    meta_priority_route_df = meta_priority_route_df.loc[
        meta_priority_route_df["type"] == priority_type
    ][["order_type_id", "idea_delta_hour"]]
    print("Done\n")

    print("3. Transform data tuyến ưu tiên...")
    df_target_modified = df_target.merge(
        meta_priority_route_df, on="order_type_id", how="inner"
    )
    ...
    preferred_carrier_df = (
        ...
        .apply(get_preferred_carrier)
        .reset_index(drop=True)
    )
    preferred_carrier_df = preferred_carrier_df.rename(
        columns={"carrier": "preferred_carrier"}
    )

    final_priority_df = grouped_priority_df.merge(
        preferred_carrier_df[
            [
                "preferred_carrier",
                "sender_province_code",
    ...
    final_priority_df["order_type"] = final_priority_df["order_type_id"].map(
        MAPPING_ID_ORDER_TYPE
    )
    final_priority_df["carrier_id"] = final_priority_df["carrier"].map(
        MAPPING_CARRIER_ID
    )
    final_priority_df["preferred_carrier_id"] = final_priority_df[
        "preferred_carrier"
    ].map(MAPPING_CARRIER_ID)
    final_priority_df["sys_order_type_id"] = final_priority_df[
        "sys_order_type_id"
    ].astype("int")
    ...
    api_priority_df = final_priority_df.loc[
        final_priority_df[
            [
                "sender_province_code",
                "sender_district_code",
        ...
        columns={"order_type_id": "new_type", "sys_order_type_id": "route_type"}
    )

    final_priority_df.to_parquet(
        ROOT_PATH + "/output/priority_route_full.parquet", index=False
    )
    api_priority_df.to_parquet(
        ROOT_PATH + "/output/priority_route_api.parquet", index=False
    )
