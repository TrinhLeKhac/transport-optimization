...
def transform_buu_cuc(buu_cuc_df, carrier="BEST Express"):
    ...
    buu_cuc_df["n_post_offices_in_province"] = buu_cuc_df.groupby("receiver_province")[
        "n_post_offices"
    ].transform(np.sum)
    buu_cuc_df["n_district"] = buu_cuc_df.groupby("receiver_province")[
        "receiver_district"
    ].transform(pd.Series.nunique)
    buu_cuc_df["status"] = buu_cuc_df.apply(
        lambda s: status_kho_giao_nhan(
            s["n_post_offices"], s["n_post_offices_in_province"], s["n_district"]
        ),
        axis=1,
    )
    buu_cuc_df["score"] = buu_cuc_df["status"].map(
        TRONG_SO["Có kho giao nhận"]["Phân loại"]
    )
...
def transform_data_kho_giao_nhan():
    ...
    buu_cuc_njv = pd.read_parquet(
        ROOT_PATH + "/processed_data/buu_cuc_ninja_van.parquet"
    )
    ...
    kho_giao_nhan = pd.concat(
        [buu_cuc_njv, buu_cuc_ghn, buu_cuc_best, buu_cuc_ghtk], ignore_index=True
    )

    # Fill thông tin default
    kho_giao_nhan = PROVINCE_MAPPING_DISTRICT_CROSS_CARRIER_DF.merge(
        kho_giao_nhan,
        on=["receiver_province", "receiver_district", "carrier"],
        how="left",
    )
    kho_giao_nhan["status"] = kho_giao_nhan["status"].fillna("Không có thông tin")
    kho_giao_nhan["score"] = kho_giao_nhan["status"].map(
        TRONG_SO["Có kho giao nhận"]["Phân loại"]
    )
