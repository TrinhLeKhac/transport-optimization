...
def score_thoi_gian_giao_hang(tong_don, thoi_gian_giao_tb, loai_van_chuyen):
    ...
    if loai_van_chuyen in ["Cách Miền"]:
        ...
        # thời gian giao trung bình < 120h
            ...
            return (
                "Trung bình dưới 120h và tổng số đơn giao hàng bé hơn hoặc bằng 5 đơn"
            )
        # trễ
            ...
            return (
                "Trung bình dưới 144h và tổng số đơn giao hàng bé hơn hoặc bằng 3 đơn"
            )
        elif (tong_don > 3) and (thoi_gian_giao_tb < 168):
            return "Trung bình dưới 168h và tổng số đơn giao hàng lớn hơn 3 đơn"
        elif (tong_don <= 3) and (thoi_gian_giao_tb < 168):
            return (
                "Trung bình dưới 168h và tổng số đơn giao hàng bé hơn hoặc bằng 3 đơn"
            )
    ...
    if loai_van_chuyen in ["Cận Miền", "Liên Miền Tp.HCM - HN", "Liên Miền Đặc Biệt"]:
        ...
        # trễ
            ...
            return (
                "Trung bình dưới 120h và tổng số đơn giao hàng bé hơn hoặc bằng 3 đơn"
            )
        elif (tong_don > 3) and (thoi_gian_giao_tb < 144):
            return "Trung bình dưới 144h và tổng số đơn giao hàng lớn hơn 3 đơn"
        elif (tong_don <= 3) and (thoi_gian_giao_tb < 144):
...
def transform_data_thoi_gian_giao_hang_toan_trinh():
    ...
    # Xử lý data
    giao_dich_thanh_cong = giao_dich_valid[
        giao_dich_valid["carrier_status"].isin(THANH_CONG_STATUS)
    ]

    giao_dich_thanh_cong = giao_dich_thanh_cong.loc[
        giao_dich_thanh_cong["last_delivering_at"].notna()
        & giao_dich_thanh_cong["picked_at"].notna()
    ]
    giao_dich_thanh_cong["delivery_time_h"] = (
        (
            giao_dich_thanh_cong["last_delivering_at"]
            - giao_dich_thanh_cong["picked_at"]
        ).dt.total_seconds()
        / 60
        / 60
    )
    ...
    giao_dich_thanh_cong_agg = (
        giao_dich_thanh_cong.groupby(
            ["receiver_province", "receiver_district", "carrier", "order_type"]
        )
        .agg(
            total_order=("delivery_time_h", "count"),
            delivery_time_mean_h=("delivery_time_h", "mean"),
        )
        .reset_index()
    )

    giao_dich_thanh_cong_agg["status"] = giao_dich_thanh_cong_agg.apply(
        lambda x: score_thoi_gian_giao_hang(
            x["total_order"], x["delivery_time_mean_h"], x["order_type"]
        ),
        axis=1,
    )

    cross_df = PROVINCE_MAPPING_DISTRICT_CROSS_CARRIER_DF.merge(
        pd.DataFrame(data={"order_type": list(MAPPING_ORDER_TYPE_ID.keys())}),
        how="cross",
    )
    giao_dich_thanh_cong_agg = cross_df.merge(
        giao_dich_thanh_cong_agg,
        on=["receiver_province", "receiver_district", "carrier", "order_type"],
        how="left",
    )

    giao_dich_thanh_cong_agg["total_order"] = (
        giao_dich_thanh_cong_agg["total_order"].fillna(0).astype(int)
    )
    giao_dich_thanh_cong_agg["delivery_time_mean_h"] = np.round(
        giao_dich_thanh_cong_agg["delivery_time_mean_h"].fillna(0), 2
    )
    giao_dich_thanh_cong_agg["status"] = giao_dich_thanh_cong_agg["status"].fillna(
        "Không có thông tin"
    )

    giao_dich_thanh_cong_agg["score"] = giao_dich_thanh_cong_agg.apply(
        lambda x: TRONG_SO["Thời gian giao hàng"]["Phân loại"][x["order_type"]][
            x["status"]
        ],
        axis=1,
    )
    giao_dich_thanh_cong_agg["criteria"] = "Thời gian giao hàng"
    giao_dich_thanh_cong_agg["criteria_weight"] = TRONG_SO["Thời gian giao hàng"][
        "Tiêu chí"
    ]
    ...
    assert (
        giao_dich_thanh_cong_agg.isna().sum().all() == 0
    ), "Transform data không chính xác"
