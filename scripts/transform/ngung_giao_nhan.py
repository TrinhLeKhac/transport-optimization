...
def transform_data_ngung_giao_nhan_level_3():
    ...
    # Đọc data ngưng giao nhận
    ngung_giao_nhan_df = pd.read_parquet(ROOT_PATH + '/processed_data/ngung_giao_nhan_level_3.parquet')

    ngung_giao_nhan_df.loc[ngung_giao_nhan_df['status'].isna(), 'status'] = 'Bình thường'
    ngung_giao_nhan_df.loc[ngung_giao_nhan_df['status'] != 'Bình thường', 'status'] = 'Quá tải'

    ngung_giao_nhan_df = (
        PROVINCE_MAPPING_DISTRICT_MAPPING_WARD_CROSS_CARRIER_DF.merge(
            ngung_giao_nhan_df, on=['receiver_province', 'receiver_district', 'receiver_commune', 'carrier'], how='left'
        )
    )
    ...
    ngung_giao_nhan_df = (
        ngung_giao_nhan_df.merge(
            PROVINCE_MAPPING_DISTRICT_MAPPING_WARD_DF.rename(columns={
                'province': 'receiver_province',
                'district': 'receiver_district',
            ...
            }), on=['receiver_province', 'receiver_district', 'receiver_commune'], how='left'
        )
    )
    ngung_giao_nhan_df['status'] = ngung_giao_nhan_df['status'].fillna('Bình thường')
    ...
    database_df = ngung_giao_nhan_df[[
        'receiver_province_code', 'receiver_district_code',
        'receiver_commune_code', 'carrier_id', 'status'
    ]]

    database_df.to_parquet(ROOT_PATH + '/transform/ngung_giao_nhan_level_3.parquet', index=False)
