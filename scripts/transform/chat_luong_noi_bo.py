...
def status_chat_luong_noi_bo_ninja_van(
    series, col1="delivery_success_rate", col2="is_more_than_100"
):
    if series[col1] == 0:
...
def transform_data_chat_luong_noi_bo_ninja_van():
    chat_luong_noi_bo_df = pd.read_parquet(
        ROOT_PATH + "/processed_data/chat_luong_noi_bo_njv.parquet"
    )

    chat_luong_noi_bo_df["status"] = chat_luong_noi_bo_df.apply(
        status_chat_luong_noi_bo_ninja_van, axis=1
    )
    chat_luong_noi_bo_df["score"] = chat_luong_noi_bo_df["status"].map(
        TRONG_SO["Chất lượng nội bộ"]["Phân loại"]["Ninja Van"]
    )

    chat_luong_noi_bo_df = chat_luong_noi_bo_df.merge(
        PROVINCE_MAPPING_DISTRICT_DF[["province", "district"]].rename(
            columns={"province": "receiver_province", "district": "receiver_district"}
        ),
        on=["receiver_province", "receiver_district"],
        how="right",
    )
    chat_luong_noi_bo_df["status"] = chat_luong_noi_bo_df["status"].fillna(
        "Không có thông tin"
    )
    chat_luong_noi_bo_df["score"] = chat_luong_noi_bo_df["status"].map(
        TRONG_SO["Chất lượng nội bộ"]["Phân loại"]["Ninja Van"]
    ...
    chat_luong_noi_bo_df.to_parquet(
        ROOT_PATH + "/transform/chat_luong_noi_bo_njv.parquet", index=False
    )
...
def transform_data_chat_luong_noi_bo_lazada():
    chat_luong_noi_bo_df = pd.read_parquet(
        ROOT_PATH + "/processed_data/chat_luong_noi_bo_lazada.parquet"
    )
    chat_luong_noi_bo_df["status"] = chat_luong_noi_bo_df.apply(
        status_chat_luong_noi_bo_lazada, axis=1
    )
    chat_luong_noi_bo_df["score"] = chat_luong_noi_bo_df["status"].map(
        TRONG_SO["Chất lượng nội bộ"]["Phân loại"]["Lazada Logistics"]
    )
    chat_luong_noi_bo_df = chat_luong_noi_bo_df.merge(
        PROVINCE_MAPPING_DISTRICT_DF[["province", "district"]].rename(
            columns={"province": "receiver_province", "district": "receiver_district"}
    ...
    chat_luong_noi_bo_df.to_parquet(
        ROOT_PATH + "/transform/chat_luong_noi_bo_lazada.parquet", index=False
    )
